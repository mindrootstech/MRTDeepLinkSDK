...
enum CliqItNetworkFailure: Error, Sendable {
    case requestFailed(String)
}

enum CliqItURLSessionRetry {
    struct HTTPResult: Sendable {
        let data: Data
        let statusCode: Int
    }

    static func data(for request: URLRequest) async -> Result<HTTPResult, CliqItNetworkFailure> {
        let maxAttempts = CliqItDefaults.apiMaxRetryAttempts
        let initialDelay = CliqItDefaults.apiRetryInitialDelay
