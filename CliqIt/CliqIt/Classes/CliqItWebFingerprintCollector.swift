...
/// Use `CliqItCombinedFingerprint` (native + web) for something that actually varies per user.
public struct CliqItWebFingerprint: Sendable, Equatable {
...
enum CliqItWebFingerprintCollector {
    private static let defaultTimeout: TimeInterval = 4.0
    private static let lock = NSLock()
    private static var _lastResult: CliqItWebFingerprint?

    /// Last successful (or empty) collect from this process.
    static var lastResult: CliqItWebFingerprint? {
        lock.lock()
        defer { lock.unlock() }
        return _lastResult
    }

    private static func store(_ value: CliqItWebFingerprint?) {
        lock.lock()
        _lastResult = value
        lock.unlock()
    }
    ...
    static func collect(
    ...
    ) async -> CliqItWebFingerprint? {
        _ = domain // domain unused — local HTML probe
        #if canImport(UIKit)
        if debugLogging {
            print("══════════════════════════════════════")
            print("🧪 [CliqIt] WEB FINGERPRINT start (inline WKWebView)")
            print("══════════════════════════════════════")
        }

        let result: CliqItWebFingerprint? = await withCheckedContinuation { continuation in
            ...
            Task {
                ...
                if debugLogging {
                    print("🧪 [CliqIt] WEB FINGERPRINT timeout (\(timeout)s)")
                }
                gate.resume(nil)
            }
        }
        ...
        if debugLogging {
            if let result {
                print("🧪 [CliqIt] WEB FINGERPRINT ok: \(result.debugDescription)")
            } else {
                print("🧪 [CliqIt] WEB FINGERPRINT failed / empty")
            }
            print("══════════════════════════════════════")
        }
        return result
    ...
    private final class ResumeGate: @unchecked Sendable {
        ...
        private var continuation: CheckedContinuation<CliqItWebFingerprint?, Never>?

        init(continuation: CheckedContinuation<CliqItWebFingerprint?, Never>) {
            self.continuation = continuation
        }

        func resume(_ value: CliqItWebFingerprint?) {
            lock.lock()
    ...
    private final class Session: NSObject, WKNavigationDelegate {
        ...
        func run(debugLogging: Bool) async -> CliqItWebFingerprint? {
        ...
        private func start() {
            ...
            if let window = keyWindow() {
                ...
                self.hostView = host
                if debugLogging {
                    print("🧪 [CliqIt] WKWebView attached (inline HTML)")
                }
            } else if debugLogging {
                print("🧪 [CliqIt] no key window — WebGL may fail")
            }
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            if debugLogging {
                print("🧪 [CliqIt] WKWebView didFinish — evaluating JS")
            }

            if #available(iOS 15.0, *) {
                webView.callAsyncJavaScript(
                    "return await window.__cliqItCollectFingerprint();",
                    arguments: [:],
                    in: nil,
                    in: .page
                ) { [weak self] result in
                    Task { @MainActor in
                        ...
                        case .success(let value):
                            if self?.debugLogging == true {
                                print("🧪 [CliqIt] JS success raw: \(String(describing: value))")
                            }
                            self?.tearDown(resumeValue: Self.parse(value), force: false)
                        case .failure(let error):
                            if self?.debugLogging == true {
                                print("🧪 [CliqIt] JS async failed: \(error.localizedDescription) — fallback sync")
                            }
        ...
        private func evaluateSyncFallback(on webView: WKWebView) {
            webView.evaluateJavaScript("window.__cliqItCollectFingerprintSync()") { [weak self] result, error in
                Task { @MainActor in
                    if let error, self?.debugLogging == true {
                        print("🧪 [CliqIt] JS sync failed: \(error.localizedDescription)")
                    }
                    if self?.debugLogging == true {
                        print("🧪 [CliqIt] JS sync raw: \(String(describing: result))")
                    }
        ...
        func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
            if debugLogging {
                print("🧪 [CliqIt] WKWebView didFail: \(error.localizedDescription)")
            }
        ...
        func webView(
            ...
            if debugLogging {
                print("🧪 [CliqIt] WKWebView provisional fail: \(error.localizedDescription)")
            }
            tearDown(resumeValue: nil, force: false)
        }

        private func tearDown(resumeValue: CliqItWebFingerprint?, force: Bool) {
        ...
        private static func parse(_ result: Any?) -> CliqItWebFingerprint? {
            let dict: [String: Any]?
            ...
            let fingerprint = CliqItWebFingerprint(
        ...
        function __cliqItFonts() {
          try {
            ...
            return __cliqItHash(detected.join(','));
          } catch (e) { return null; }
        }
        ...
        window.__cliqItCollectFingerprintSync = function() {
        ...
        window.__cliqItCollectFingerprint = async function() {
          var sync = window.__cliqItCollectFingerprintSync();
          sync.audioHash = await __cliqItAudio();
          return sync;
        };
        </script></body></html>
        """
