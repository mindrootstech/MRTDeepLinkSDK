...
public struct MRTCombinedFingerprint: Sendable, Equatable {
...
enum MRTCombinedFingerprintBuilder {
    static func build(web: MRTWebFingerprint?) -> MRTCombinedFingerprint {
        #if canImport(UIKit)
        ...
        var parts: [String: String] = [
            ...
            "pixelRatioBucket": MRTInstallDeviceInfo.devicePixelRatioBucket(),
        ...
        if let v = MRTInstallDeviceInfo.currencyCode() { parts["currency"] = v }
        if let v = MRTInstallDeviceInfo.regionCode() { parts["regionCode"] = v }
        if let v = MRTInstallDeviceInfo.mapContentSizeCategory() { parts["dynamicTypeSize"] = v }
        ...
        return MRTCombinedFingerprint(digest: digest, parts: parts)
    }
}
